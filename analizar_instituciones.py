...
def analizar_instituciones():
    ...
    for tipo in tipos:
        ...
        instituciones = Institucion.objects.filter(
            tipo_institucion=tipo, eliminado=False
        )
        ...
        if tipo == "particular":
            ...
            for campo in campos_vacios_esperados:
                count_no_vacio = (
                    instituciones.exclude(**{campo: None})
                    .exclude(**{campo: ""})
                    .count()
                )
                if count_no_vacio > 0:
                    print(
                        f"  ⚠️  Campo '{campo}' tiene valores en {count_no_vacio} registros"
                    )
                    # Obtener ejemplos
                    ejemplos = (
                        instituciones.exclude(**{campo: None})
                        .exclude(**{campo: ""})
                        .values_list("id", "nombre", campo)[:3]
                    )
                    for ej in ejemplos:
            ...
            for campo in campos_con_valor_esperados:
                count_vacio = (
                    instituciones.filter(**{campo: None}).filter(**{campo: ""}).count()
                )
                if count_vacio > 0:
                    print(f"  ⚠️  Campo '{campo}' está vacío en {count_vacio} registros")
                    ejemplos = (
                        instituciones.filter(**{campo: None})
                        .filter(**{campo: ""})
                        .values_list("id", "nombre")[:3]
                    )
                    for ej in ejemplos:
                        inconsistencias.append(
                            {
        ...
        # Para 'educativa', codigo_mppe debería tener valor
            ...
            count_sin_codigo_mppe = (
                instituciones.filter(codigo_mppe__isnull=True)
                .filter(codigo_mppe="")
                .count()
            )
            if count_sin_codigo_mppe > 0:
                print(
                    f"  ⚠️  Campo 'codigo_mppe' está vacío en {count_sin_codigo_mppe} registros"
                )
                ejemplos = (
                    instituciones.filter(codigo_mppe__isnull=True)
                    .filter(codigo_mppe="")
    ...
    if inconsistencias:
        for inc in inconsistencias[:20]:  # Limitar a 20 ejemplos
            print(
                f"Tipo: {inc['tipo']}, Campo: {inc['campo']}, ID: {inc['id']}, Nombre: {inc['nombre']}, Valor: {inc['valor']}"
            )
    else:
