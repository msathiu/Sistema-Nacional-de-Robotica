...
sys.path.insert(
    0, "/home/argenis/Escritorio/MiProyecto/sistemaRobotica/SistemaRegistro"
)
...
from registry.models import Estado, Municipio, Parroquia, Institucion, Dependencia
...
def crear_datos_basicos():
    ...
    parroquia, _ = Parroquia.objects.get_or_create(
        nombre="San Juan", municipio=municipio
    )
...
def test_reactivacion_institucion_eliminada():
    ...
    estado, municipio, parroquian = crear_datos_basicos()
    ...
    print(
        f"✓ Institución creada y eliminada: '{institucion_eliminada.nombre}' (ID: {institucion_eliminada.id})"
    )
    ...
    if is_valid:
        ...
        print(
            f"  Es la misma institución: {institucion_guardada.id == institucion_eliminada.id}"
        )
        ...
        reactivacion_correcta = (
            institucion_guardada.id == institucion_eliminada.id
            and institucion_guardada.eliminado == False  # Misma institución
            and institucion_guardada.activa == False  # Reactivada
            and institucion_guardada.estatus  # Inhabilitada hasta aprobación
...
def test_reactivacion_particular_eliminado():
    ...
    print(
        f"✓ Persona natural creada y eliminada: '{particular_eliminado.nombre}' (ID: {particular_eliminado.id})"
    )
    ...
    if is_valid:
        ...
        reactivacion_correcta = (
            institucion_guardada.id == particular_eliminado.id
            and institucion_guardada.eliminado == False
            and institucion_guardada.activa == False
            and institucion_guardada.estatus == "pendiente"
        )
...
def main():
    ...
    try:
        results.append(
            ("Reactivación Educativa", test_reactivacion_institucion_eliminada())
        )
        results.append(
            ("Reactivación Particular", test_reactivacion_particular_eliminado())
        )
        results.append(
            ("No Reactivar Datos Diferentes", test_no_reactivacion_datos_diferentes())
        )
