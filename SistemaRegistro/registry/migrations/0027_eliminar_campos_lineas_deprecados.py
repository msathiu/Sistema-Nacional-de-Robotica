...
def verificar_migracion_lineas(apps, schema_editor):
    ...
    if not campos_existen:
        ...
        if clubes_sin_lineas > 0:
            print(
                f"Advertencia: {clubes_sin_lineas} clubes no tienen líneas de investigación."
            )
        else:
            print(
                "Todos los clubes tienen líneas de investigación correctamente configuradas."
            )
        return
    ...
    if clubes_migrados > 0:
        print(
            f"Migración completada: {clubes_migrados} clubes migrados a ClubLineaInvestigacion"
        )
    else:
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(
            verificar_migracion_lineas, reverse_code=migrations.RunPython.noop
        ),
