...
def dashboard_metricas_clubes(request):
    ...
    tasa_aprobacion = (
        (clubes_aprobados / total_procesados * 100) if total_procesados > 0 else 0
    )
    ...
    if es_central:
        ...
        for estado in Estado.objects.all():
            count = clubes_base.filter(
                institucion_creadora__estado=estado, status="aprobado", activo=True
            ).count()
            if count > 0:
        ...
        if perfil.estado:
            count = clubes_base.filter(status="aprobado", activo=True).count()
            if count > 0:
                clubes_por_estado.append(
                    {"estado": perfil.estado.nombre, "count": count}
                )

    # Tiempo promedio de revisión
    ultimos_30_dias = timezone.now() - timedelta(days=30)
    clubes_aprobados_recientes = clubes_base.filter(
        status="aprobado", fecha_aprobacion__gte=ultimos_30_dias
    )

    resultado = (
        clubes_aprobados_recientes.filter(
            fecha_aprobacion__isnull=False, fecha_creacion__isnull=False
        )
        .annotate(
    ...
    clubes_populares = (
        clubes_base.filter(status="aprobado", activo=True)
        .annotate(
            num_membresias=Count(
                "membresias", filter=Q(membresias__estado="miembro_activo")
            )
...
def exportar_clubes_csv(request):
    ...
    clubes = (
        Club.objects.filter(status="aprobado", activo=True, eliminado=False)
    ...
    for club in clubes:
        ...
        writer.writerow(
            [
                ...
                club.fecha_aprobacion.strftime("%d/%m/%Y")
                if club.fecha_aprobacion
                else "",
            ]
        )
        ...
        data.append(
            {
                ...
                "fecha_aprobacion": club.fecha_aprobacion.isoformat()
                if club.fecha_aprobacion
                else None,
            }
        )
...
def exportar_equipos_excel(request):
    ...
    qs = (
        ...
        .select_related("institucion", "institucion__estado", "evento")
    ...
    rows = []
    for g in qs:
        tutores = ", ".join(f"{t.nombres} {t.apellidos}" for t in g.tutores.all())
        rows.append(
            [
                g.codigo,
                g.nombre,
                ...
                g.institucion.estado.nombre
                if g.institucion and g.institucion.estado
                else "",
                tutores,
    ...
    for vi in qs:
        ...
        rows.append(
            [
                ...
                vi.institucion.estado.nombre
                if vi.institucion and vi.institucion.estado
                else "",
                vi.rol or "",
    ...
    for ins in qs:
        ...
        rows.append(
            [
                ...
                if hasattr(ins, "fecha_inscripcion") and ins.fecha_inscripcion
                else "",
            ]
        )
