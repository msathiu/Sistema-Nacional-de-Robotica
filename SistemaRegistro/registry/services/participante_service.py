...
class ParticipanteService:
    ...
    @staticmethod
    def onboard_participante(
        username, email, password, datos_personales, institucion_inicial=None
    ):
        """
    ...
    def crear_participante(
        datos_participante: dict, creado_por_federacion: bool = False
    ) -> Participante:
        """
        ...
        if "cedula" in datos_participante and datos_participante["cedula"]:
            datos_participante["cedula"] = "".join(
                filter(str.isdigit, datos_participante["cedula"])
            )

        if (
            "cedula_escolar" in datos_participante
            and datos_participante["cedula_escolar"]
        ):
            datos_participante["cedula_escolar"] = "".join(
                filter(str.isdigit, datos_participante["cedula_escolar"])
            )
        ...
        logger.info(
            f"[Participante] Nuevo participante creado: {participante.nombre_completo} ({participante.cedula})"
        )
        return participante
    ...
    def vincular_participante_institucion(
        ...
        if not created:
            ...
            if vinculacion.status != "activo":
                ...
                logger.info(
                    f"[Participante] Vinculación reactivada: {participante} @ {institucion}"
                )
            else:
                logger.info(
                    f"[Participante] Vinculación ya existe: {participante} @ {institucion}"
                )
        else:
            logger.info(
                f"[Participante] Nueva vinculación: {participante} @ {institucion}"
            )
    ...
    def registrar_participante_con_institucion(
        ...
        with transaction.atomic():
            ...
            if datos_participante.get("cedula"):
                participante = ParticipanteService.buscar_por_cedula(
                    datos_participante["cedula"]
                )

            if not participante and datos_participante.get("cedula_escolar"):
                participante = ParticipanteService.buscar_por_cedula_escolar(
                    datos_participante["cedula_escolar"]
                )

            participante_creado = False

            if not participante:
                participante = ParticipanteService.crear_participante(
                    datos_participante
                )
            ...
            if grupo:
                ParticipanteGrupo.objects.get_or_create(
                    participante=participante, grupo=grupo, defaults={"activo": True}
                )
    ...
    @staticmethod
    def asignar_a_grupo(
        participante: Participante, grupo: Grupo, institucion: Institucion
    ):
        """
        ...
        try:
            vinculacion = ParticipanteInstitucion.objects.get(
                participante=participante, institucion=institucion, status="activo"
            )
        ...
        logger.info(
            f"[Participante] {participante.nombre_completo} asignado al grupo {grupo.nombre}"
        )
    ...
    def obtener_participantes_por_institucion(
        institucion: Institucion, solo_activos: bool = True
    ):
        """
        ...
        """
        queryset = ParticipanteInstitucion.objects.select_related(
            "participante"
