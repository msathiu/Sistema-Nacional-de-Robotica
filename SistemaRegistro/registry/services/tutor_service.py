...
class TutorService:
    ...
    def crear_tutor(datos_tutor: dict, creado_por_federacion: bool = False) -> Tutor:
        ...
        logger.info(
            f"[Tutor] Nuevo tutor creado: {tutor.get_nombre_completo()} ({cedula_limpia})"
        )
        return tutor
    ...
    def vincular_tutor(
        ...
        if tipo_vinculacion == "institucional":
            if not institucion:
                raise ValidationError(
                    "Se requiere una institución para vinculación institucional."
                )
            filtros["institucion"] = institucion
        elif tipo_vinculacion == "regional":
            if not estado:
                raise ValidationError(
                    "Se requiere un estado para vinculación regional."
                )
        ...
        vinculacion, created = TutorInstitucion.objects.get_or_create(
            **filtros, defaults={"rol": rol, "status": "activo"}
        )
        ...
        if not created:
            if vinculacion.status != "activo":
                ...
                logger.info(
                    f"[Tutor] Vinculación reactivada: {tutor} @ {tipo_vinculacion}"
                )
        else:
        ...
        with transaction.atomic():
            ...
            if not tutor:
                tutor = TutorService.crear_tutor(
                    datos_tutor, creado_por_federacion=creado_por_federacion
                )
                tutor_creado = True
    ...
    def asignar_tutor_a_grupo(
        tutor: Tutor, grupo: Grupo, usuario: Optional[User] = None
    ) -> None:
        """
        ...
        if grupo.institucion:
            vinculacion_activa = TutorInstitucion.objects.filter(
                tutor=tutor, institucion=grupo.institucion, status="activo"
            ).exists()
    ...
    def cambiar_estado_tutor(
        ...
        if not vinculacion:
            try:
                ...
                raise ValidationError(
                    "Ambigüedad en la vinculación. Especifique el ID de vinculación."
                )
        ...
        logger.info(
            f"[Tutor] Estado cambiado a {nuevo_status} para vinculación {vinculacion.id}"
        )
        return vinculacion
