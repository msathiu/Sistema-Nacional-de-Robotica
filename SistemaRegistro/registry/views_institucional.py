...
from .models import (
    Grupo, Participante, Evento, Club, MembresiaClu,
    InscripcionGrupoEvento, Institucion
)
...
def eliminar_grupo(request, grupo_id):
    ...
    if request.method == 'POST':
        ...
        if grupo.estado_grupo != 'editable':
            messages.error(request, 'No se puede eliminar un grupo inscrito o bloqueado.')
        else:
            nombre = grupo.nombre
            grupo.delete()
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                ...
                messages.success(request, f'Grupo "{grupo.nombre}" inscrito exitosamente.')
        except Exception as e:
