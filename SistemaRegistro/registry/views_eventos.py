...
def enviar_evento_revision(request, evento_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f'Evento "{evento.nombre}" enviado a revisión correctamente.'
        )
        return redirect("eventos_club", club_id=evento.club_organizador.id)
...
def revisar_eventos_club(request):
    ...
    eventos_revision = (
        Evento.objects.pendientes_aprobacion()
        .select_related("club_organizador", "creado_por")
...
def aprobar_evento_club(request, evento_id):
    ...
    if request.method == "POST":
        ...
        if not observaciones:
            ...
            return render(
                request, "registry/aprobar_evento_club.html", {"evento": evento}
            )
...
def rechazar_evento_club(request, evento_id):
    ...
    if evento.estado_evento != EstadoEvento.REVISION:
        messages.error(
            request, "Este evento no puede ser rechazado en su estado actual."
        )
        return redirect("revisar_eventos_club")
    ...
    if request.method == "POST":
        ...
        if not observaciones:
            ...
            return render(
                request, "registry/rechazar_evento_club.html", {"evento": evento}
            )
...
def detalle_evento_club(request, evento_id):
    ...
    inscripciones = (
        evento.inscripciones_grupo.filter(activo=True)
