    ...
    def wrapper(request, *args, **kwargs):
        ...
        if membresia_id:
            ...
            membresia = get_object_or_404(
                MembresiaClu.objects.select_related("club"), id=membresia_id
            )
            club = membresia.club
        elif club_id:
        ...
        if not hasattr(request.user, "userprofile"):
            messages.error(
                request, "No tienes un perfil institucional asociado a tu cuenta."
            )
            return redirect("clubes_lista")

        if not request.user.userprofile.institution:
            messages.error(
                request, "Tu perfil no está vinculado a ninguna institución."
            )
            return redirect("clubes_lista")
...
def es_ente_rector(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not hasattr(request.user, "userprofile"):
            messages.error(request, "No tienes un perfil asociado a tu cuenta.")
...
def puede_ver_membresia(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        membresia = get_object_or_404(
            MembresiaClu.objects.select_related(
                "club", "institucion", "club__institucion_creadora"
            ),
            id=membresia_id,
        )
        ...
        if not tiene_permiso:
            raise PermissionDenied("No tienes permiso para ver esta membresía.")
