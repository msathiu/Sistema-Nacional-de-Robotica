...
def calificar_club(request, club_id):
    ...
    if (
        ...
        or not request.user.userprofile.institution
    ):
...
def vincular_club_evento(request, club_id):
    ...
    if request.method == "POST":
        ...
        vinculacion, created = ClubEvento.objects.get_or_create(
            club=club, evento=evento, defaults={"rol": rol}
        )

        if created:
            messages.success(
                request, f"Club vinculado al evento '{evento.nombre}' exitosamente."
            )
        else:
    ...
    eventos_disponibles = Evento.objects.filter(
        activo=True, fecha__gte=timezone.now().date()
    ).exclude(clubes_vinculados__club=club)

    return render(
        request,
        "registry/vincular_club_evento.html",
        {
            "club": club,
...
def clubes_eliminados(request):
    ...
    ) or request.user.userprofile.user_type not in ["fed_central", "superuser"]:
        messages.error(request, "No tiene permisos para acceder a esta sección.")
        return redirect("dashboard")

    clubes = (
        Club.objects.filter(eliminado=True)
        .select_related("institucion_creadora", "eliminado_por")
        .order_by("-fecha_eliminacion")
    )

    return render(
        request,
        "registry/clubes_eliminados.html",
        {
            "clubes": clubes,
        },
    )
...
def eliminar_permanente_club(request, club_id):
    ...
    if not hasattr(
        ...
        messages.error(
            request, "No tiene permisos para eliminar permanentemente clubes."
        )
        return redirect("dashboard")
    ...
    return render(
        request,
        "registry/eliminar_permanente_club.html",
        {
            "club": club,
        },
    )
