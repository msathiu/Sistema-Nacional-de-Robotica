...
class GrupoForm(forms.ModelForm):
    ...
    class Meta:
        model = Grupo
        fields = ['nombre', 'criterio', 'edad_desde', 'edad_hasta', 'nivel_educativo', 'nombre_proyecto']
        widgets = {
            ...
            'edad_desde': forms.NumberInput(attrs={
                'class': 'form-control',
                'placeholder': 'Ej: 8',
                'min': '4',
                'max': '99'
            }),
            'edad_hasta': forms.NumberInput(attrs={
                'class': 'form-control',
                'placeholder': 'Ej: 12',
                'min': '4',
                'max': '99'
    ...
    def clean(self):
        ...
        if criterio == 'edad':
            ...
            if not edad_desde:
                self.add_error('edad_desde', 'Este campo es obligatorio para criterio "Por Edad"')
            if not edad_hasta:
                self.add_error('edad_hasta', 'Este campo es obligatorio para criterio "Por Edad"')
            
            if edad_desde and edad_hasta:
                if edad_desde > edad_hasta:
                    self.add_error('edad_desde', 'La edad desde no puede ser mayor que edad hasta')
                if edad_desde < 4:
            ...
            if not nivel_educativo:
                self.add_error('nivel_educativo', 'Este campo es obligatorio para criterio "Por Nivel Educativo"')
                
        elif criterio == 'proyecto':
            nombre_proyecto = cleaned_data.get('nombre_proyecto', '').strip()
            if not nombre_proyecto:
                self.add_error('nombre_proyecto', 'Este campo es obligatorio para criterio "Por Proyecto"')
            else:
