...
class TutorHierarchyTestCase(TestCase):
    def setUp(self):
        ...
        self.estado_miranda, _ = Estado.objects.get_or_create(
            nombre="Miranda", codigo="MI"
        )

        self.municipio = Municipio.objects.create(
            estado=self.estado_zulia, nombre="Maracaibo"
        )
        self.parroquia = Parroquia.objects.create(
            municipio=self.municipio, nombre="Olego"
        )
    ...
    def test_vinculacion_central_exitosa(self):
        ...
        vinc, created = TutorService.vincular_tutor(
            tutor=self.tutor, tipo_vinculacion="central"
        )
    ...
    def test_vinculacion_regional_exitosa(self):
        ...
        vinc, created = TutorService.vincular_tutor(
            tutor=self.tutor, tipo_vinculacion="regional", estado=self.estado_zulia
        )
        self.assertTrue(created)
    ...
    def test_multiples_vinculaciones_diferentes_entes(self):
        ...
        TutorService.vincular_tutor(
            self.tutor, tipo_vinculacion="regional", estado=self.estado_zulia
        )
        # Institucional
        TutorService.vincular_tutor(
            self.tutor, tipo_vinculacion="institucional", institucion=self.institucion
        )
    ...
    def test_constraint_regional_por_estado(self):
        ...
        # Permitir Miranda
        TutorInstitucion.objects.create(
            tutor=self.tutor, tipo_vinculacion="regional", estado=self.estado_miranda
        )
    ...
    def test_service_vincular_tutor_reactiva_inactivo(self):
        ...
        vinc = TutorInstitucion.objects.create(
            tutor=self.tutor, tipo_vinculacion="central", status="inactivo"
        )

        vinc_updated, created = TutorService.vincular_tutor(
            self.tutor, tipo_vinculacion="central"
        )
    ...
    def test_vincular_regional_sin_estado_falla(self):
        ...
        with self.assertRaises(ValidationError):
            TutorService.vincular_tutor(
                self.tutor, tipo_vinculacion="regional", estado=None
            )
    ...
    def test_vincular_institucional_sin_institucion_falla(self):
        ...
        with self.assertRaises(ValidationError):
            TutorService.vincular_tutor(
                self.tutor, tipo_vinculacion="institucional", institucion=None
            )
