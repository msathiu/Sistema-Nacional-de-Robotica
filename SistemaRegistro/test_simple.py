...
def test_reactivacion():
    ...
    print(
        f"Institución eliminada creada: {inst_eliminada.nombre} (ID: {inst_eliminada.id})"
    )
    ...
    if is_valid:
        ...
        if (
            inst_guardada.id == inst_eliminada.id
            and inst_guardada.eliminado == False
            and inst_guardada.activa == False
            and inst_guardada.estatus == "pendiente"
        ):
