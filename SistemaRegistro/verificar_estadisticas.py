...
def verificar_estadisticas():
    ...
    profile_fed = UserProfile.objects.create(
        user=user_fed,
    ...
    print(f"✅ Evento cancelado creado: {evento_cancelado.nombre} (ID: {evento_cancelado.id})")
    ...
    if response.status_code == 200:
        ...
        print(f"\n🎯 Verificación:")
        print(f"   - Pausados esperados: {esperado_pausados}, obtenidos: {stats['pausados']} {'✅' if stats['pausados'] == esperado_pausados else '❌'}")
        print(f"   - Cancelados esperados: {esperado_cancelados}, obtenidos: {stats['cancelados']} {'✅' if stats['cancelados'] == esperado_cancelados else '❌'}")
        print(f"   - Total esperados: {esperado_total}, obtenidos: {stats['total']} {'✅' if stats['total'] == esperado_total else '❌'}")
        ...
        if 'data-target="' in content:
            ...
            if target_match:
                ...
                print(f"   - Template renderizado correctamente: {'✅' if '2' in target_value else '❌'}")
