...
class LocationFormMixin:
    ...
    def setup_location_fields(self):
        ...
        estado_id = (
            self.data.get("estado")
            or (self.instance.estado_id if self.instance.pk else None)
            or self.initial.get("estado")
        )
        municipio_id = (
            self.data.get("municipio")
            or (self.instance.municipio_id if self.instance.pk else None)
            or self.initial.get("municipio")
        )
        parroquia_id = (
            self.data.get("parroquia")
            or (self.instance.parroquia_id if self.instance.pk else None)
            or self.initial.get("parroquia")
        )

        if estado_id:
            try:
                municipio_qs = Municipio.objects.filter(estado_id=estado_id).order_by(
                    "nombre"
                )
                ...
                if municipio_id:
                    try:
                        ...
                        if municipio_obj not in municipio_qs:
                            municipio_qs = (
                                Municipio.objects.filter(pk=municipio_obj.pk)
                                | municipio_qs
                            )
        ...
        if municipio_id:
            try:
                parroquia_qs = Parroquia.objects.filter(
                ...
                if parroquia_id:
                    try:
                        ...
                        if parroquia_obj not in parroquia_qs:
                            parroquia_qs = (
                                Parroquia.objects.filter(pk=parroquia_obj.pk)
                                | parroquia_qs
                            )
    ...
    def clean_location_integrity(self, cleaned_data):
        ...
        if estado and municipio and municipio.estado_id != estado.id:
            self.add_error(
                "municipio",
                f"El municipio '{municipio}' no pertenece al estado '{estado}'.",
            )

        if municipio and parroquia and parroquia.municipio_id != municipio.id:
            self.add_error(
                "parroquia",
                f"La parroquia '{parroquia}' no pertenece al municipio '{municipio}'.",
            )
...
class ParticipanteBaseFormMixin:
    ...
    def clean_id_fields(self, cleaned_data):
        ...
        cedula_personal = cleaned_data.get(
            "cedula_personal"
        ) or StringUtils.clean_numeric_id(self.data.get("cedula_personal", ""))
        cedula_escolar = cleaned_data.get(
            "cedula_escolar_input"
        ) or StringUtils.clean_numeric_id(self.data.get("cedula_escolar_input", ""))
        ...
        if edad < 18:
            ...
            for campo in campos_rep:
                if not cleaned_data.get(campo):
                    self.add_error(
                        campo, "Este campo es obligatorio para menores de edad."
                    )
