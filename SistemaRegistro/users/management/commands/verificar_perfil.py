...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for perfil in perfiles:
            ...
            self.stdout.write(
                f'  Estado: {perfil.estado.nombre if perfil.estado else "Sin asignar"}'
            )
            self.stdout.write(
                f'  Institution: {perfil.institution.nombre if perfil.institution else "Sin asignar"}'
            )
        ...
        pendientes = Institucion.objects.filter(
        ...
        if pendientes.exists():
            for inst in pendientes[:10]:  # Mostrar solo las primeras 10
                self.stdout.write(
                    f'  - {inst.nombre} (Estado: {inst.estado.nombre if inst.estado else "Sin estado"})'
                )
