...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'  activa=False: {instituciones.filter(activa=False).count()}')
        ...
        self.stdout.write(f'  instituciones.filter(estatus="pendiente", activa=False): {pendientes_y_no_activas.count()}')
        
        # alternative: solo por estatus="pendiente"
        solo_pendientes = instituciones.filter(estatus='pendiente')
        self.stdout.write(f'  instituciones.filter(estatus="pendiente"): {solo_pendientes.count()}')
