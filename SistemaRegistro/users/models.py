...
class UserProfile(models.Model):
    ...
    user_type = models.CharField(
        max_length=25, choices=USER_TYPES, default="participante"
    )
...
def create_or_update_user_profile(sender, instance, created, **kwargs):
    ...
    try:
        if created:
            ...
            if (
                hasattr(instance, "_skip_profile_creation")
                and instance._skip_profile_creation
            ):
                return
            ...
            profile, was_created = UserProfile.objects.get_or_create(
                user=instance, defaults={"user_type": user_type}
            )
            if not was_created:
                logger.warning(
                    f"El perfil para el usuario {instance.username} ya existía"
                )
        else:
            ...
            try:
                ...
                logger.error(
                    f"Error al actualizar perfil de usuario {instance.username}: {e}"
                )
    except Exception as e:
        # Manejar cualquier error de integridad de base de datos
        logger.error(
            f"Error en señal create_or_update_user_profile para {instance.username}: {e}"
        )
