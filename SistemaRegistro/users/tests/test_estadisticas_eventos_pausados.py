...
class EstadisticasEventosPausadosTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.profile_fed = UserProfile.objects.create(
            user=self.user_fed, user_type="fed_central"
        )
        ...
        self.institucion = Institucion.objects.create(
            nombre="Institución Test", codigo="INST001", estado=self.estado
        )
    ...
    def test_estadisticas_template_render(self):
        ...
        self.assertIn(
            'data-target="{{ stats.pausados|add:stats.cancelados|default:0 }}"', content
        )

        # Verificar que se muestre el número correcto
        self.assertIn(
            'data-target="2"', content
        )  # 2 pausados + 1 cancelado = 3, pero el template muestra el total
