...
class GestionarEstadoFixTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.institucion = Institucion.objects.create(
            nombre="Institución Test", codigo="INST001", estado_id=1
        )
    ...
    def test_pausar_evento_abierto(self):
        ...
        self.assertEqual(
            self.evento_abierto.observacion_estado, "Pausa por condiciones climáticas"
        )
    ...
    def test_reabrir_evento_pausado(self):
        ...
        self.assertEqual(
            self.evento_pausado.observacion_estado, "Condiciones normalizadas"
        )
    ...
    def test_cancelar_evento(self):
        ...
        self.assertEqual(
            self.evento_abierto.observacion_estado,
            "Cancelación por falta de participantes",
        )
    ...
    def test_finalizar_evento(self):
        ...
        self.assertEqual(
            self.evento_abierto.observacion_estado, "Evento concluido exitosamente"
        )
    ...
    def test_reprogramar_evento(self):
        ...
        self.evento_abierto.refresh_from_db()
        self.assertEqual(
            self.evento_abierto.estado_evento, EstadoEvento.ABIERTO
        )  # Estado sin cambios
        self.assertEqual(self.evento_abierto.fecha, nueva_fecha)
        self.assertEqual(
            self.evento_abierto.observacion_estado,
            "Reprogramado por solicitud de participantes",
        )
    ...
    def test_accion_no_soportada_error_message(self):
        ...
        response = self.client.post(
            url, {"estado_evento": "accion_inexistente", "observacion": "Test"}
        )
        ...
        self.assertTrue(
            any("no soportado para esta acción" in str(msg) for msg in messages)
        )
    ...
    def test_reprogramar_sin_fecha_error(self):
        ...
        response = self.client.post(
            url,
            {"estado_evento": "reprogramar", "observacion": "Reprogramar sin fecha"},
        )
        ...
        messages = list(response.wsgi_request.messages)
        self.assertTrue(
            any("nueva fecha para reprogramar" in str(msg) for msg in messages)
        )
    ...
    def test_pausar_sin_observacion_error(self):
        ...
        messages = list(response.wsgi_request.messages)
        self.assertTrue(
            any("observación visible al pausar" in str(msg) for msg in messages)
        )
...
class GestionarEstadoIntegrationTestCase(TestCase):
    ...
    def test_flujo_completo_evento(self):
        ...
        # 1. Pausar evento
        response = self.client.post(
            url, {"estado_evento": "pausado", "observacion": "Pausa temporal"}
        )
        self.assertEqual(response.status_code, 302)
        ...
        # 2. Reabrir evento
        response = self.client.post(
            url, {"estado_evento": "abierto", "observacion": "Reapertura"}
        )
        self.assertEqual(response.status_code, 302)
        ...
        # 3. Finalizar evento
        response = self.client.post(
            url, {"estado_evento": "finalizado", "observacion": "Evento concluido"}
        )
        self.assertEqual(response.status_code, 302)
