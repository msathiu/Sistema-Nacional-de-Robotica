...
def register(request):
    ...
    if request.method == "POST":
        ...
        if user_form.is_valid() and participante_form.is_valid():
            ...
            messages.success(
                request, f"Participante {user.username} registrado exitosamente."
            )
...
def dashboard(request):
    ...
    if user_type == "participante":
        ...
        conteo_db = Institucion.objects.values("estado__nombre").annotate(
            total=Count("id")
        )
        mapa_data = {
...
def dashboard_institucional(request):
    ...
    eventos_asignados = (
        Evento.objects.filter(grupos_inscritos__usuario_creador=usuario)
        .distinct()
        .count()
    )
...
def lista_instituciones(request):
    ...
    instituciones_activas = instituciones_base.filter(
        activa=True
    ).count()  # Corregido el nombre
    ...
    for institucion in instituciones_base:
        ...
        usuarios_institucionales = User.objects.filter(
            userprofile__institution=institucion, userprofile__user_type="institucional"
        )

        instituciones_con_usuarios.append(
            {"institucion": institucion, "usuarios": usuarios_institucionales}
        )
...
def registrar_institucion(request):
    ...
    es_administrador = (
        ...
        and request.user.userprofile.user_type == "admin"
    )
            ...
            try:
                with transaction.atomic():
                    ...
                    if es_administrador:
                        messages.success(
                            request,
                            f"La sede '{institucion.nombre}' ha sido registrada y activada correctamente.",
                        )
    ...
    return render(
        request,
        "users/registrar_institucion.html",
        {
            "form": form,
            "base_template": base_template,
...
def lista_participantes(request):
    ...
    context = {
        ...
        == "admin",  # Para mostrar/ocultar botones en el HTML
    }
...
def crear_evento(request):
    ...
    return render(request, "users/crear_evento.html", {"estados": estados})
...
def eventos_disponibles(request):
    ...
    context = {"estados_con_eventos": estados_con_eventos}
...
def aprobar_institucion(request, institucion_id):
    with transaction.atomic():
        ...
        perfil = (
            UserProfile.objects.filter(institution=institucion)
            .select_related("user")
            .first()
        )
        if perfil and perfil.user:
            ...
            messages.success(
                request,
                f"La cuenta de {institucion.nombre} ha sido activada correctamente.",
            )
        else:
            messages.warning(
                request,
                f"Institución activada, pero no se encontró un usuario vinculado.",
            )
...
def desactivar_institucion(request, institucion_id):
    with transaction.atomic():
        ...
        messages.warning(
            request, f'La institución "{inst.nombre}" y su acceso han sido suspendidos.'
        )
    return redirect("lista_instituciones")
        ...
        if user:
            ...
            if nueva_clave:
                if nueva_clave == confirm_clave:
                    ...
                    messages.warning(
                        request,
                        f"La institución se actualizó, pero las contraseñas no coincidían.",
                    )
            else:
...
def gestionar_eventos_institucion(request):
    ...
    eventos = (
        Evento.objects.filter(institucion=institucion)
        .annotate(total_inscritos=Count("inscripcion"))
        .order_by("-fecha")
    )

    return render(
        request,
        "users/gestionar_eventos.html",
        {"eventos": eventos, "institucion": institucion},
    )
