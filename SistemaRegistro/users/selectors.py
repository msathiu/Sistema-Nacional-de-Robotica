...
class JurisdictionSelector:
    ...
    ROLES_RECTORES = ["fed_central", "tecnologico"]
    ROLES_FEDERACION = ["fed_central", "fed_regional", "tecnologico"]
    ...
    def es_federacion(perfil):
        return (
            getattr(perfil, "user_type", None) in JurisdictionSelector.ROLES_FEDERACION
        )

    @staticmethod
    def filtrar_por_territorio(
        queryset, perfil, campo_estado="estado", campo_institucion="institucion"
    ):
        """
...
class EventoSelector:
    ...
    def get_eventos_visibles(user_or_perfil):
        ...
        # Eventos de fed_central cancelados: visibles para trazabilidad
        # (instituciones con equipos inscritos deben poder ver el estado final)
        eventos_fed_central_cancelados = Evento.objects.filter(
            cancelado=True,
        ...
        # fed_central y tecnologico ven todo lo publicable.
        ...
        # Incluye también eventos nacionales de fed_central sin estado geográfico asignado.
        ...
        if user_type == "institucional":
            ...
            # NOTA: NO excluir eventos con institucion=NULL (son de fed_central, deben ser visibles)
            ...
            return (eventos_visibles | eventos_fed_central_cancelados).distinct()
...
class ParticipanteSelector:
    ...
    def get_participantes_para_perfil(perfil):
        ...
        # Para federación central: retornar TODOS los participantes
        # (la lógica de vinculaciones se manejará en la vista)
        if user_type == "fed_central":
            return Participante.objects.all()
        ...
        if user_type == "fed_regional" and perfil.estado:
            return Participante.objects.filter(
                Q(
                    vinculaciones__tipo_vinculacion="regional",
                    vinculaciones__estado=perfil.estado,
                    vinculaciones__status="activo",
                )
                | Q(
                    vinculaciones__tipo_vinculacion="institucional",
                    vinculaciones__institucion__estado=perfil.estado,
                    vinculaciones__status="activo",
                )
            ).distinct()

        # Para instituciones: retornar SOLO participantes vinculados a su institución
        if user_type == "institucional" and perfil.institution:
            return Participante.objects.filter(
    ...
    @staticmethod
    def get_participantes_visibles_para_usuario(user):
        """
        Retorna el queryset de participantes visibles para acciones puntuales
        de detalle/edición/eliminación sin depender de rutas o middleware.
        """
        ...
        if user_type == "participante":
            return Participante.objects.filter(user=user)
...
class InstitucionSelector:
    ...
    def get_stats_instituciones(queryset):
        ...
        return {"total": total, "activas": activas, "pendientes": total - activas}
