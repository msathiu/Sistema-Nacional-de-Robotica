...
class SedeRegionalForm(forms.Form):
    ...
    username = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "ej: sede_miranda"}
        ),
    )
    email = forms.EmailField(
        label="Correo",
        widget=forms.EmailInput(
            attrs={"class": "form-control", "placeholder": "sede@correo.com"}
        ),
    )
    ...
    nombres = forms.CharField(
        label="Nombres",
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Nombres del encargado"}
        ),
    )
    apellidos = forms.CharField(
        label="Apellidos",
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Apellidos del encargado"}
        ),
    )
    cedula = forms.CharField(
        label="Cédula",
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "V-12345678"}
        ),
    )
...
class ParticipanteModalEditForm(forms.ModelForm):
    ...
    numero_telefono = forms.CharField(
        ...
        min_length=7,
        widget=forms.TextInput(
            attrs={"class": "form-control border-0 shadow-sm", "maxlength": "7"}
        ),
    )
    ...
    def clean_numero_telefono(self):
        ...
        if numero and not re.match(r"^\d{7}$", numero):
            raise ValidationError(
    ...
    def clean(self):
        cleaned_data = super().clean()
        # Validaciones básicas cross-field si es necesario
        return cleaned_data
...
class InstitucionModalEditForm(forms.Form):
    ...
    rif_numero = forms.CharField(max_length=20, required=False)
    modal_cod_area = forms.ChoiceField(choices=CODIGO_AREA_CHOICES, required=False)
    modal_num_puro = forms.CharField(max_length=7, required=False)
    ...
    def clean_rif_numero(self):
        rif_numero = StringUtils.clean_numeric_id(self.cleaned_data.get("rif_numero"))
        return rif_numero or ""

    def clean_modal_num_puro(self):
        numero = StringUtils.clean_numeric_id(self.cleaned_data.get("modal_num_puro"))
        if not numero:
            return ""
        if len(numero) != 7:
    ...
    def clean(self):
        ...
        cod_area = cleaned_data.get("modal_cod_area")
        numero = cleaned_data.get("modal_num_puro")
    ...
    def save(self):
        if not self.instance:
            raise ValueError("InstitucionModalEditForm requiere una instancia.")

        self.instance.nombre = self.cleaned_data["nombre"]
        self.instance.email = self.cleaned_data["email"]
        self.instance.direccion = self.cleaned_data["direccion"]

        rif_letra = self.cleaned_data.get("rif_letra")
        rif_numero = self.cleaned_data.get("rif_numero")
        if rif_letra and rif_numero:
            self.instance.rif = f"{rif_letra}-{rif_numero}"

        cod_area = self.cleaned_data.get("modal_cod_area")
        numero = self.cleaned_data.get("modal_num_puro")
        if cod_area and numero:
            self.instance.telefono_codigo = cod_area
            self.instance.telefono_numero = numero
            self.instance.telefono = f"{cod_area}{numero}"
...
class EventoContactDataForm(forms.Form):
    ...
    def clean_telefono_numero(self):
        raw_value = (self.cleaned_data.get("telefono_numero") or "").strip()
        if not raw_value:
            return ""
        ...
        if re.search(r"[^0-9\\s().-]", raw_value):
            raise forms.ValidationError(
                "El teléfono de contacto contiene caracteres no permitidos."
            )

        numero = StringUtils.clean_numeric_id(raw_value)
        if len(numero) != 7:
            raise forms.ValidationError(
                "El teléfono de contacto debe tener exactamente 7 dígitos."
            )
...
class ParticipanteRegistrationForm(
    ParticipanteBaseFormMixin, LocationFormMixin, forms.ModelForm
):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._seed_vinculacion_defaults_for_edit()
    ...
    def _configure_fields_by_role(self):
        ...
        if self.user_role == "fed_central":
            ...
            if self.user_institution:
                ...
                ].queryset = Institucion.objects.filter(id=self.user_institution.id)
                self.fields[
                    "vinculacion_institucion"
                ].initial = self.user_institution.id
    ...
    def _seed_vinculacion_defaults_for_edit(self):
        """
        En edición, conserva la vinculación vigente aunque el template no envíe
        explícitamente los campos ocultos.
        """
        if not getattr(self.instance, "pk", None):
            return

        vinculacion_activa = (
            self.instance.vinculaciones.filter(status="activo")
        ...
        default_tipo = None
        default_institucion = None
        default_estado = None

        if self.user_role == "institucional" and self.user_institution:
            default_tipo = "institucional"
            default_institucion = self.user_institution
        elif vinculacion_activa:
            default_tipo = vinculacion_activa.tipo_vinculacion
            default_institucion = vinculacion_activa.institucion
            default_estado = vinculacion_activa.estado

        if default_tipo and not self.initial.get("tipo_vinculacion"):
            self.initial["tipo_vinculacion"] = default_tipo
            self.fields["tipo_vinculacion"].initial = default_tipo

        if default_institucion and not self.initial.get("vinculacion_institucion"):
            self.initial["vinculacion_institucion"] = default_institucion.pk
            self.fields["vinculacion_institucion"].initial = default_institucion.pk

        if default_estado and not self.initial.get("vinculacion_estado"):
            self.initial["vinculacion_estado"] = default_estado.pk
            self.fields["vinculacion_estado"].initial = default_estado.pk

        if self.is_bound:
            data = self.data.copy()
            if default_tipo and not data.get("tipo_vinculacion"):
                data["tipo_vinculacion"] = default_tipo
            if default_institucion and not data.get("vinculacion_institucion"):
                data["vinculacion_institucion"] = str(default_institucion.pk)
            if default_estado and not data.get("vinculacion_estado"):
                data["vinculacion_estado"] = str(default_estado.pk)
            self.data = data
    ...
    def clean_cedula_escolar_input(self):
        val = StringUtils.clean_numeric_id(
            self.cleaned_data.get("cedula_escolar_input")
        )
        return val if val else None

    def clean_cedula_representante(self):
        cedula = StringUtils.clean_numeric_id(
            self.cleaned_data.get("cedula_representante")
        )
        if cedula and len(cedula) > 10:
            raise forms.ValidationError(
                "La cédula del representante no puede exceder los 10 dígitos."
            )
        if cedula and len(cedula) < 7:
            raise forms.ValidationError(
                "La cédula del representante debe tener al menos 7 dígitos."
            )
        return cedula
    ...
    def clean(self):
        ...
        if self.user_role == "fed_central":
            ...
            if tipo_vinculacion == "institucional" and not institucion:
                raise ValidationError(
                    "Debe seleccionar una institución para la vinculación institucional."
                )

            if tipo_vinculacion == "regional" and not estado:
                raise ValidationError(
                    "Debe seleccionar un estado para la vinculación regional."
                )
...
class InstitucionRegistrationForm(LocationFormMixin, forms.ModelForm):
    ...
    particular_nombres = forms.CharField(
        max_length=100, required=False, label="Nombres"
    )
    particular_apellidos = forms.CharField(
        max_length=100, required=False, label="Apellidos"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        tipo_institucion = self.initial.get("tipo_institucion") or self.data.get(
            "tipo_institucion"
        )
        if tipo_institucion:
    ...
    def _configure_fields_by_tipo_institucion(self, tipo_institucion):
        ...
        if tipo_institucion == "particular":
            ...
            ].initial = "V"  # En persona natural, nombre puede derivarse de los datos de particular
            self.fields["nombre"].required = False

    def _handle_institucion_eliminada(
        self, institucion_eliminada, campo_validado, valor_campo
    ):
        """
    ...
    def _validar_datos_coinciden(self, institucion_eliminada, cleaned_data):
        ...
        # Para personas naturales
            ...
            # Validar cédula
            cedula = StringUtils.clean_numeric_id(
                cleaned_data.get("particular_cedula", "")
            )
    ...
    def _buscar_institucion_eliminada_para_reactivar(self, cleaned_data):
        ...
        institucion = Institucion.objects.filter(email=email, eliminado=True).first()
    ...
    def clean_email(self):
        ...
        if existing:
            raise forms.ValidationError(
                "Ya existe una institución registrada con este correo."
            )
    ...
    def clean_numero_telefono(self):
        """
        Normaliza y valida el número telefónico local.
        Solo se admiten 7 dígitos para conservar el contrato actual del formulario.
        """
        raw_value = (self.cleaned_data.get("numero_telefono") or "").strip()
        if not raw_value:
            raise forms.ValidationError("El número telefónico es obligatorio.")

        if re.search(r"[A-Za-z]", raw_value):
            raise forms.ValidationError(
                "El número telefónico debe contener solo dígitos."
            )

        if re.search(r"[^0-9\s().-]", raw_value):
            raise forms.ValidationError(
                "El número telefónico contiene caracteres no permitidos."
            )

        numero = StringUtils.clean_numeric_id(raw_value)
        if len(numero) != 7:
            raise forms.ValidationError(
                "El número telefónico debe tener exactamente 7 dígitos."
            )
    ...
    def clean(self):
        ...
        institucion_eliminada = self._buscar_institucion_eliminada_para_reactivar(
            cleaned_data
        )
        ...
        if tipo_institucion == "particular":
            if not cleaned_data.get("particular_nombres"):
                self.add_error(
                    "particular_nombres",
                    "Los nombres son obligatorios para personas naturales.",
                )
            if not cleaned_data.get("particular_apellidos"):
                self.add_error(
                    "particular_apellidos",
                    "Los apellidos son obligatorios para personas naturales.",
                )
                ...
                if existing:
                    if existing.eliminado:
                        ...
                        self._handle_institucion_eliminada(
                            existing, "particular_cedula", cedula
                        )
                    else:
                        # Validación normal: institución activa ya existe
                        self.add_error(
                            "particular_cedula", f"Cédula {cedula} ya registrada."
                        )
        ...
        if len(password) < 8:
            ...
            if not re.search(r'[!@#$%^&*()_\-=\[\]{};:\'",.<>?/\\|`~]', password):
                self.add_error(
                    "password", "Debe incluir un carácter especial (!@#$%^&*...)."
                )
    ...
    def save(self, commit=True):
        ...
        if tipo_institucion == "particular":
            ...
            instance.particular_apellidos = self.cleaned_data.get(
                "particular_apellidos"
            )
            instance.particular_nacionalidad = self.cleaned_data.get(
                "particular_nacionalidad"
            )
...
class ClubRegistrationForm(forms.ModelForm):
    ...
    from registry.models import ClubLineaInvestigacion, LineaInvestigacion
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            for i, l in enumerate(
                [
                    self.cleaned_data.get("linea_investigacion_1"),
                    self.cleaned_data.get("linea_investigacion_2"),
                ]
            ):
                if l:
                    ClubLineaInvestigacion.objects.create(
                        club=club, linea=l, orden=i + 1
                    )
        return club
