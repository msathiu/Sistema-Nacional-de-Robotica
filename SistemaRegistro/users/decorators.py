...
def admin_access_required(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        messages.error(
            request, "Solo superusuarios pueden acceder al panel de administración."
        )
        return redirect("dashboard")
...
def admin_required(view_func):
    ...
    return role_required(["admin", "fed_central", "fed_regional", "tecnologico"])(
        view_func
    )
        ...
        if user_type in ["admin", "fed_central", "tecnologico", "fed_regional"]:
            ...
            if user_type == "fed_regional":
                ...
                if inst.estado != user_profile.estado:
                    messages.error(
                        request,
                        "No tienes permiso sobre instituciones de otros estados.",
                    )
