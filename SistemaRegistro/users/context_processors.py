...
def sidebar_menu(request):
    ...
    if es_central:
        ...
        header_title = (
            f"Sede Regional: {perfil.estado.nombre}"
            if getattr(perfil, "estado", None)
            else "Sede Regional"
        )
        rol_label = "AUDITOR"
        perfil_url = "mi_perfil_federacion"
...
def _menu_central(is_active, is_expanded, request=None):
    ...
    menu.append(
        {
            ...
            "url_name": "mapa_interactivo",
            "icon": "bi-map-fill",
            "active": is_active("mapa_interactivo"),
        }
    )
        ...
        menu.append(
            {
                ...
                "type": "submenu",
                "items": items,
                "expanded": is_expanded(label, items),
            }
        )
    menu.append(
        {
            "label": "Eventos y Actividades",
            "url_name": "admin_eventos",
            "icon": "bi-calendar-event",
            "active": is_active("admin_eventos", "admin_todos_eventos"),
        }
    )
    ...
    reportes_items = [
        {
            "label": "Participantes",
            "url_name": "exportar_participantes_excel",
            "icon": "bi-people",
            "active": is_active("exportar_participantes_excel"),
        },
        {
            "label": "Equipos",
            "url_name": "exportar_equipos_excel",
            "icon": "bi-microsoft-teams",
            "active": is_active("exportar_equipos_excel"),
        },
        {
            "label": "Tutores",
            "url_name": "exportar_tutores_excel",
            "icon": "bi-person-workspace",
            "active": is_active("exportar_tutores_excel"),
        },
        {
            "label": "Instituciones",
            "url_name": "exportar_instituciones_excel",
            "icon": "bi-building",
            "active": is_active("exportar_instituciones_excel"),
        },
        {
            "label": "Inscripciones",
            "url_name": "exportar_inscripciones_excel",
            "icon": "bi-calendar-check",
            "active": is_active("exportar_inscripciones_excel"),
        },
    ]
    menu.append(
        {
            ...
            "expanded": is_expanded("Reportes", reportes_items),
        }
    )
    return menu
...
def _menu_regional(is_active, is_expanded):
    ...
    return [
        ...
        },
        {
            ...
            "icon": "bi-building",
            "active": is_active("lista_instituciones"),
        },
        {
            "label": "Participantes",
            "url_name": "lista_participantes",
            "icon": "bi-people",
            "active": is_active("lista_participantes"),
        },
        {
            "label": "Métricas",
            "url_name": "dashboard_metricas_clubes",
            "icon": "bi-graph-up",
            "active": is_active("dashboard_metricas_clubes"),
        },
        {
            "label": "Reportes",
            "icon": "bi-file-earmark-spreadsheet",
...
def _menu_institucional(is_active, is_expanded, request=None):
    ...
    for label, icon, children in submenus:
        items = [
            {"label": c[0], "url_name": c[1], "icon": c[2], "active": is_active(c[1])}
            for c in children
        ]
        menu.append(
            {
    ...
    menu.append(
        {
            ...
            "url_name": "mis_notificaciones",
            "icon": "bi-bell",
            "active": is_active("mis_notificaciones"),
            "badge": notif_count or None,
        }
    )
    ...
    menu.append(
        {
            ...
            "type": "submenu",
            "items": reportes_inst,
            "expanded": is_expanded("Reportes", reportes_inst),
        }
    )
    return menu
